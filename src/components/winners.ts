...
class Winners {
  async createWinners(winnerList: Array<Winner>): Promise<void> {
    ...
    winnerList.forEach((elem: Winner) => {
      winnersPromises.push(apiController.getCarInfoForWinners(elem.id));
    });

    carsInfo = (await Promise.all(winnersPromises)).flat() as Array<Car>;
