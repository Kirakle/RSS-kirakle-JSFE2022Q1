...
export default class {
  ...
  setUpdateListener(): void {
    const updateText = document.getElementById('update-text') as HTMLInputElement;
    const updateColor = document.getElementById('update-color') as HTMLInputElement;
    const updateButton = document.getElementById('update') as HTMLDivElement;
  ...
  setGenerateListener(): void {
    const generator = document.getElementById('generate') as HTMLDivElement;
  ...
  setPageListeners(): void {
    const prevButton = document.getElementById('prev') as HTMLDivElement;
  ...
  setDriveListeners(): void {
    ...
    startButtons.forEach((startButton: Node) => {
      (startButton as HTMLDivElement).addEventListener(
        'click',
        async (): Promise<void> => {
          const id = ((startButton as HTMLDivElement).dataset.start as unknown) as number;
          if (!(startButton as HTMLDivElement).classList.contains('disabled')) {
  ...
  async driveStarter(id: number): Promise<void> {
    const startButton = document.querySelector(`[data-start="${id}"]`) as HTMLDivElement;
    const stopButton = document.querySelector(`[data-stop="${id}"]`) as HTMLDivElement;
  ...
  setStopListeners(): void {
    ...
    stopButtons.forEach((stopButton: Node) => {
      (stopButton as HTMLDivElement).addEventListener('click', async () => {
        const id = ((stopButton as HTMLDivElement).dataset.stop as unknown) as number;
        if (!(stopButton as HTMLDivElement).classList.contains('disabled')) {
  ...
  startRace(): Array<Promise<void>> {
    ...
    buttons.forEach((elem: Node) => {
      const id = +(((elem as HTMLDivElement).dataset.start as unknown) as number);
  ...
  setRaceListener(): void {
    const raceButton = document.getElementById('race') as HTMLDivElement;
  ...
  stopRace(): Array<Promise<void>> {
    ...
    buttons.forEach((elem: Node) => {
      const id = +(((elem as HTMLDivElement).dataset.stop as unknown) as number);
  ...
  setHideWinnerListener(): void {
    const winner = document.querySelector('.winnerbox') as HTMLDivElement;
  ...
  setNumberListener(): void {
    const number = document.querySelector('#win-num') as HTMLDivElement;
    const wins = document.querySelector('#win-win') as HTMLDivElement;
    const time = document.querySelector('#win-time') as HTMLDivElement;
  ...
  setWinnersPageListeners(): void {
    const prevButton = document.getElementById('win-prev') as HTMLDivElement;
    const nextButton = document.getElementById('win-next') as HTMLDivElement;
